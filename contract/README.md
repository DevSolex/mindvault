...
| --- | --- | --- | --- |
| `register(creator, id, price, metadata, tags)` | `creator` | `Result<(), Error>` | Register a listed resource with normalized tags and no content hash. |
| `register_with_hash(creator, id, price, metadata, tags, content_hash)` | `creator` | `Result<(), Error>` | Register a resource with an optional immutable content hash. |
| `set_price(id, new_price)` | `creator` | `Result<(), Error>` | Update a mutable resource's price. |
| `update_metadata(id, metadata)` | `creator` | `Result<(), Error>` | Update the metadata pointer while it is not frozen. |
| `freeze_metadata(id)` | `creator` | `Result<(), Error>` | Permanently freeze the metadata pointer and record `metadata_frozen_at`. |
| `set_tags(id, tags)` | `creator` | `Result<(), Error>` | Replace normalized discovery tags. |
| `set_listed(id, listed)` | `creator` | `Result<(), Error>` | Toggle listed/delisted state through the boolean projection. |
| `delist(id)` | `creator` | `Result<(), Error>` | Convenience wrapper for `set_listed(id, false)`. |
| `freeze_resource(id)` | `creator` | `Result<(), Error>` | Move a listed or delisted resource into lifecycle `Frozen`. |
| `open_dispute(id, admin)` | `admin` | `Result<(), Error>` | Move listed, delisted, or frozen resources into lifecycle `Disputed`. |
| `resolve_dispute(id, admin, state)` | `admin` | `Result<(), Error>` | Resolve a disputed resource to listed, delisted, or frozen. |
| `tombstone_resource(id, admin)` | `admin` | `Result<(), Error>` | Permanently retire a resource and remove it from tag discovery. |
| `transfer_ownership(id, new_creator)` | `creator` | `Result<(), Error>` | Transfer ownership immediately. |
| `propose_transfer(id, new_creator)` | `creator` | `Result<(), Error>` | Propose a two-step ownership transfer. |
| `accept_transfer(id)` | proposed `new_creator` | `Result<(), Error>` | Accept a pending transfer. |
| `cancel_transfer(id)` | `creator` | `Result<(), Error>` | Cancel a pending transfer. |
| `get(id)` | none | `Result<Resource, Error>` | Read a resource by id. |
| `get_many(ids)` | none | `Result<Vec<Option<Resource>>, Error>` | Batch read resources in input order. |
| `exists(id)` | none | `bool` | Return whether a resource exists. |
| `exists_many(ids)` | none | `Vec<bool>` | Batch existence check in input order. |
| `get_owner(id)` | none | `Result<Address, Error>` | Read the current owner for a resource. |
| `count()` | none | `u32` | Return the monotonic registration count. |
| `creator_resource_count(creator)` | none | `u32` | Return the current number of resources owned by a creator. |
| `list(start, limit)` | none | `Vec<Resource>` | List resources in registration order, capped at `LIST_PAGE_CAP`. |
| `list_page(cursor, limit)` | none | `CatalogPage` | List resources with a next-cursor response. |
| `list_listed(start, limit)` | none | `Vec<Resource>` | List resources currently in lifecycle `Listed`. |
| `list_by_creator(creator, start, limit)` | none | `Vec<Resource>` | List resources currently owned by a creator. |
| `list_by_tag(tag, start, limit)` | none | `Vec<Resource>` | List non-tombstoned resources carrying a normalized tag. |
| `list_by_dispute_status(flagged, start, limit)` | none | `Vec<Resource>` | List resources by whether `dispute_flag` is active. |
| `registry_info()` | none | `RegistryInfo` | Return registry discovery metadata. |
| `contract_version()` | none | `ContractVersion` | Return crate and resource schema versions. |
| `initialize_network(network_id)` | none | `Result<(), Error>` | Bind the registry to the current ledger network id. |
| `network_id()` | none | `Result<BytesN<32>, Error>` | Return the initialized network id. |
| `admin()` | none | `Option<Address>` | Return the current admin. |
| `pending_admin()` | none | `Option<Address>` | Return the pending admin nomination. |
| `nominate_new_admin(new_admin)` | current admin or bootstrap `new_admin` | `Result<(), Error>` | Bootstrap or nominate an admin. |
| `accept_admin(new_admin)` | pending admin | `Result<(), Error>` | Accept an admin nomination. |
| `set_paused(admin, paused)` | `admin` | `Result<(), Error>` | Set the emergency pause flag. |
| `is_paused()` | none | `bool` | Return the emergency pause flag. |
| `add_verifier(verifier)` | `admin` | `Result<(), Error>` | Grant verifier role. |
| `remove_verifier(verifier)` | `admin` | `Result<(), Error>` | Revoke verifier role. |
| `is_verifier(address)` | none | `bool` | Return whether an address holds verifier role. |
| `set_verification_status(id, verifier, status)` | `verifier` | `Result<(), Error>` | Mirror off-chain verification status on-chain. |
| `add_settler(settler)` | `admin` | `Result<(), Error>` | Grant settler role. |
| `remove_settler(settler)` | `admin` | `Result<(), Error>` | Revoke settler role. |
| `is_settler(address)` | none | `bool` | Return whether an address holds settler role. |
| `add_moderator(moderator)` | `admin` | `Result<(), Error>` | Grant moderator role. |
| `remove_moderator(moderator)` | `admin` | `Result<(), Error>` | Revoke moderator role. |
| `is_moderator(address)` | none | `bool` | Return whether an address holds moderator role. |
| `flag_resource(id, moderator, reason)` | `moderator` | `Result<(), Error>` | Set an active dispute flag. |
| `unflag_resource(id, moderator)` | `moderator` | `Result<(), Error>` | Clear an active dispute flag. |
| `set_flag_reason_hash(id, moderator, reason_hash)` | `moderator` | `Result<(), Error>` | Store an off-chain dispute reason hash. |
| `get_flag_reason_hash(id)` | none | `Result<String, Error>` | Read the stored dispute reason hash. |
| `set_terms_hash(creator, terms_hash)` | `creator` | `Result<(), Error>` | Store a creator terms hash. |
| `get_terms_hash(creator)` | none | `Result<String, Error>` | Read a creator terms hash. |
| `set_fee_config(config)` | `admin` | `Result<(), Error>` | Store registry fee configuration. |
| `get_fee_config()` | none | `Option<FeeConfig>` | Read registry fee configuration. |
| `repair_index(ids)` | `admin` | `Result<(), Error>` | Rebuild the global pagination index. |
| `repair_tag_index(ids)` | `admin` | `Result<(), Error>` | Rebuild derived tag indexes. |
| `record_payment(settler, receipt_id, resource_id, payer, amount, tx_hash)` | `settler` and `payer` | `Result<(), Error>` | Store an escrowed payment receipt under a unique receipt id. |
| `settle_payment(settler, receipt_id)` | `settler` | `Result<(), Error>` | Mark an escrowed payment receipt as settled. |
| `get_payment(receipt_id)` | none | `Result<PaymentReceipt, Error>` | Read a payment receipt by receipt id. |
| `get_payment_receipt(resource_id, payer)` | none | `Result<PaymentReceipt, Error>` | Read the latest payment receipt for a resource/payer pair. |
| `anchor_purchase_receipt(service, resource_id, buyer, receipt_hash)` | `verifier` | `Result<(), Error>` | Anchor an immutable purchase receipt hash. |
| `get_purchase_receipt(resource_id, buyer)` | none | `Result<PurchaseReceiptAnchor, Error>` | Read an anchored purchase receipt hash. |
| `extend_resource_ttl(creator, resource_id)` | `creator` | `Result<(), Error>` | Refresh resource persistent storage TTL. |
...
| `MAX_RECEIPT_ID_LEN`       | `64`                         | Maximum length of a payment receipt id in bytes.      |
